...
export const Scene4 = () => {
    ...
    const loadPlayer = () => {
        gltfLoader.current.load(SoldierModel, (gltf) => {
            ...
            // let mesh = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.1, 0.1), new THREE.MeshBasicMaterial({color: 'green'}));
            model.add(camera.current)
            person.current = model;
            // _scene.current.add(mesh);
            let shape = new CANNON.Box(new CANNON.Vec3(0.1, 0.1, 0.1));
    ...
    const tick = (scene) => {
        ...
        objectsToUpdate.current.forEach(object => {
            object.mesh.position.copy(object.body.position).y = object.body.position.y - 0.14
    ...
    const updateMovement = () => {
        ...
        /**
         * Movement through velocity (rigid bodies)
         * */
        if (canJump.current) {
